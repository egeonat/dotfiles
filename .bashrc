...
__conda_setup="$('/kuacc/apps/anaconda/3.19.10/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/kuacc/apps/anaconda/3.19.10/etc/profile.d/conda.sh" ]; then
        . "/kuacc/apps/anaconda/3.19.10/etc/profile.d/conda.sh"
    else
        export PATH="/kuacc/apps/anaconda/3.19.10/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
...
# Stationary SSH_AUTH_SOCK location so it doesn't get outdated in tmux sessions
...
# Load modules if module command available
